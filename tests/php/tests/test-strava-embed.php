...
class Test_Strava_Embed extends WP_UnitTestCase {
	 ...
	 * @covers Block_For_Strava_Embed::render_strava_embed
	 */
	public function test_render_strava_embed_appends_token_for_activity(): void {
		$content = $this->makeEmbedBlockContent( 'https://www.strava.com/activities/18233733854' );
		$block   = array(
			'attrs' => array(
				'providerNameSlug' => 'strava',
	 ...
	 * Strava's share dialog can ship a `data-token` for routes too, and the
	 * snippet-paste path round-trips it; this test pins that the renderer
	 * doesn't drop one in favor of the other.
	 *
	 * @covers Block_For_Strava_Embed::render_strava_embed
	 */
	public function test_render_strava_embed_token_coexists_with_route_params(): void {
		$content = $this->makeEmbedBlockContent( 'https://www.strava.com/routes/456' );
		$block   = array(
			'attrs' => array(
				'providerNameSlug'    => 'strava',
	 ...
	 * Strava's embed page handles inconsistently. Stay with the clean
	 * URL shape when no token has been resolved.
	 *
	 * @covers Block_For_Strava_Embed::render_strava_embed
	 */
	public function test_render_strava_embed_skips_empty_token(): void {
		$content = $this->makeEmbedBlockContent( 'https://www.strava.com/activities/123' );
		$block   = array(
			'attrs' => array(
				'providerNameSlug' => 'strava',
	 ...
	 * @covers Block_For_Strava_Embed::render_strava_embed
	 */
	public function test_render_strava_embed_url_only_when_no_token_attribute(): void {
		$content = $this->makeEmbedBlockContent( 'https://www.strava.com/activities/12345' );
		$block   = array(
			'attrs' => array(
				'providerNameSlug' => 'strava',
	 ...
	 * @covers Block_For_Strava_Embed::render_strava_embed
	 */
	public function test_render_strava_embed_makes_no_http_calls_for_activities(): void {
		$http_calls = 0;
		$callback   = static function ( $preempt, $args, $url ) use ( &$http_calls ) {
		...
		Block_For_Strava_Embed::render_strava_embed( $content, $block );
	...
	/**
	 * 403 from strava-embeds.com is the "needs token" signal — the endpoint
	 * must report `embeddable: false` so the editor can show its notice.
	 *
	 * @covers Block_For_Strava_Embed::register_rest_routes
	 */
	...
	/**
	 * Endpoint must reject anonymous traffic — the preflight causes outbound
	 * HTTP, so leaving it open to unauthenticated callers would hand the
	 * world a free reflective fetch.
	 *
	 * @covers Block_For_Strava_Embed::register_rest_routes
	 */
	 ...
	 * a route and an activity can share a numeric ID space (Strava
	 * doesn't promise otherwise) and a key collision would silently
	 * serve the wrong probe result across types. Pins the type prefix
	 * before someone "simplifies" `md5( $type . ':' . $id )` to
	 * `md5( $id )`.
	 *
	 * @covers Block_For_Strava_Embed::register_rest_routes
	 */
