...
async function waitForEditor( page: Page ): Promise< void > {
	...
	await page.locator( 'iframe[name="editor-canvas"]' ).waitFor( { timeout: 15000 } );
...
test.describe.serial( 'Strava Activity block', () => {
	...
	test( 'frontend render: large style is rendered', async ( { page } ) => {
		...
		const placeholder = page.locator( '.wp-block-obenland-strava-activity .strava-embed-placeholder' );
		await expect( placeholder ).toBeAttached();
